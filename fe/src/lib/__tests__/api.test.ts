...
describe("pollJob", () => {
  it("sends a GET to /api/jobs/test-id", async () => {
    const jobResponse: JobResponse = { job_id: "test-id", status: "processing" };
    mockResponse(200, jobResponse);

    await pollJob("test-id");

    expect(mockFetch).toHaveBeenCalledWith("/api/jobs/test-id", expect.anything());
  });
  ...
  it("throws Error on 404", async () => {
    ...
    await expect(pollJob("test-id")).rejects.toThrow("Request failed with status 404");
  });
...
describe("saveRating", () => {
  it("sends a POST to /api/save", async () => {
    ...
    await saveRating({ job_id: "abc-123", rating: 4, processing_time_ms: 1200 });
  ...
  it("resolves void on 200", async () => {
    ...
    const result = await saveRating({ job_id: "abc-123", rating: 4, processing_time_ms: 1200 });
...
describe("fetchStats", () => {
  ...
  it("throws Error on non-2xx", async () => {
    ...
    await expect(fetchStats()).rejects.toThrow("Request failed with status 500");
  });
});
