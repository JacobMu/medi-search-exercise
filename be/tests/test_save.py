...
async def test_save_success(
    ...
    response = await app_client.post(
        "/save",
        json={"job_id": job_id, "rating": 4, "processing_time_ms": 500},
    )
...
async def test_save_rating_too_low(app_client: AsyncClient) -> None:
    response = await app_client.post(
        "/save",
        json={"job_id": "any-id", "rating": 0, "processing_time_ms": 100},
    )
...
async def test_save_rating_too_high(app_client: AsyncClient) -> None:
    response = await app_client.post(
        "/save",
        json={"job_id": "any-id", "rating": 6, "processing_time_ms": 100},
    )
    assert response.status_code == 422


async def test_save_negative_processing_time(app_client: AsyncClient) -> None:
    response = await app_client.post(
        "/save",
        json={"job_id": "any-id", "rating": 3, "processing_time_ms": -1},
    )
    assert response.status_code == 422
